...
from draw_board import SaveDrawnBoard
...
def main():
    ...
    if 'CONFIG' not in st.session_state:
        ...
        st.session_state['BOARD'] = {'N_ROWS': 5, 'N_COLS': 5} # Set the BOARD Size at 5x5
        st.session_state['FILL_PRCT'] = 75 # Set the percentage of agents depending on the BOARD
        ...
        st.session_state['DONE_SETUP'] = False
    ...
    if not st.session_state['P_IN_PROGRESS']:
        ...
        if not st.session_state['DONE_SETUP']:
            with st.spinner('Threads are being launched...'):
                st.session_state['DONE_SETUP'] = True
                st.session_state['P_INIT_TIME'], st.session_state['P_COUNT'] = time.time(), 1
                ...
                # ---- Launch THREADS ----
                Agent.nbRow, Agent.nbCol = st.session_state['BOARD']['N_ROWS'], st.session_state['BOARD']['N_COLS']
                ...
                SaveDrawnBoard(Agent.nbRow, Agent.nbCol, Agent.agentDict, st.session_state['PATH_FONT'], st.session_state['PATH_FOLDER'], 0)
                st.session_state['PATH_INIT_IM'] = f'{st.session_state["PATH_FOLDER"]}/PuzzleMA-{Agent.nbRow}_{Agent.nbCol}-Im_0.png'
                
                st.session_state['AGENT_LIST'] = list(Agent.agentDict.values())
                for agent in st.session_state['AGENT_LIST']:
                    agent.start()
             
        # ---- Run PROCESS ----             
        if SaveDrawnBoard(Agent.nbRow, Agent.nbCol, Agent.agentDict, st.session_state['PATH_FONT'], st.session_state['PATH_FOLDER'], st.session_state['P_COUNT']) == True:
            st.session_state['P_COUNT'] += 1

        areaPlaceholder.empty()
        st.session_state['PATH_SLCT_IM'] = f'{st.session_state["PATH_FOLDER"]}/PuzzleMA-{Agent.nbRow}_{Agent.nbCol}-Im_{st.session_state["P_COUNT"]-1}.png'
        with areaPlaceholder.container():
            ...
            st.markdown(f"""
            <div style="text-align:center; margin-top:10px; margin-bottom:10px">
                <h3 style="font-size:20px; font-family: monospace; margin-bottom:10px">Process in Progress... (since {round(time.time() - st.session_state["P_INIT_TIME"])}s)</h3>
            </div>
            """, unsafe_allow_html=True)
            
            if st.button('STOP EXECUTION'):
                st.session_state['LIMIT_TIME'] = 0
            ...
            col2.markdown(f"""
                <div style="text-align:center; margin-top:10px; margin-bottom:-20px">
                    <h3 style="font-size:20px; font-family: monospace">Puzzle - <span style="color:red">STEP {st.session_state["P_COUNT"]-1}</span></h3>
                </div>
                """, unsafe_allow_html=True)                    
        ...
        if Agent.verifyRunning() or ((time.time() - st.session_state['P_INIT_TIME']) >= (st.session_state['LIMIT_TIME'] * 60)):
            # Turn of the Running
            for agent in st.session_state['AGENT_LIST']:
    ...
    with areaPlaceholder.container():
        ...
        if len(st.session_state['LIST_IMAGES']) == 1:
            ImagePath = f'{st.session_state["PATH_FOLDER"]}/PuzzleMA-{st.session_state["BOARD"]["N_ROWS"]}_{st.session_state["BOARD"]["N_COLS"]}-Im_{st.session_state["INDEX_RIGHT_IMAGE"][0]}.png'
            _, col1, _ = st.columns([4.5,3,4.5])
            col1.markdown(f"""
                <div style="text-align:center">
                    <h4 style="font-weight:bold">PUZZLE BOARD</h4>
                </div>
                """, unsafe_allow_html=True)
            ...
            ImageInitPath = f'{st.session_state["PATH_FOLDER"]}/PuzzleMA-{st.session_state["BOARD"]["N_ROWS"]}_{st.session_state["BOARD"]["N_COLS"]}-Im_{st.session_state["INDEX_LEFT_IMAGE"]}.png'
            ImagePath = f'{st.session_state["PATH_FOLDER"]}/PuzzleMA-{st.session_state["BOARD"]["N_ROWS"]}_{st.session_state["BOARD"]["N_COLS"]}-Im_{st.session_state["INDEX_RIGHT_IMAGE"]}.png'
            ...
            col1.markdown(f"""
                <div style="text-align:center">
                    <h4 style="font-weight:bold">BOARD {left}</h4>
                </div>
            """, unsafe_allow_html=True)
            col2.markdown(f"""
                <div style="text-align:center">
                    <h4 style="font-weight:bold">BOARD {right}</h4>
                </div>
                """, unsafe_allow_html=True)
            col3.markdown(f"""
                <div style="text-align:center">
